...
<table class="info-layout">
    <tr>
        <td style="width: 35%;">
            ...
            <table class="meta-data-table">
                <tr>
                    ...
                    <td class="meta-value">{{ optional($salesInvoice->invoice_date)->format('d/m/Y') ?? '—' }}</td>
                </tr>
                <tr>
                    <td class="meta-label">{{ $isAr ? 'أمر البيع' : 'Sales Order' }}</td>
                    <td class="meta-value">{{ optional($salesInvoice->salesOrder)->so_number ?? '—' }}</td>
                </tr>
                <tr>
        ...
        <td style="width: 30%; text-align: center; vertical-align: bottom; padding-bottom: 15px;">
            <div class="client-phone-line">{{ optional($salesInvoice->client)->phone ?? '' }}</div>
        </td>

        <td style="width: 35%; text-align: {{ $isAr ? 'left' : 'right' }};">
            <div class="block-label">{{ $isAr ? 'مفوترة إلى' : 'BILLED TO' }}</div>
            <div class="client-company-name">{{ optional($salesInvoice->client)->company_name ?? '—' }}</div>
            <div class="client-details-text">{{ optional($salesInvoice->client)->contact_person ?? '' }}</div>
        </td>
    </tr>
</table>
...
<table class="items-data-table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="col-center">{{ number_format($line->unit_price, 2) }}</td>
    ...
    <tfoot>
        <tr>
            <td colspan="3" class="col-directional">
                {{ $isAr ? 'الإجمالي' : 'Total' }} <span style="font-weight: normal; color: #94a3b8; font-size: 9px;">({{ $salesInvoice->items->count() }} {{ $isAr ? 'صنف' : 'items' }})</span>
            </td>
            ...
            <td class="col-directional" style="color: #005B9F; font-weight: bold; font-size: 12px;">
                {{ number_format($salesInvoice->grand_total, 2) }} <span style="font-size: 8px; color: #94a3b8; font-weight: normal;">{{ $salesInvoice->currency }}</span>
            </td>
        </tr>
    </tfoot>
</table>
    ...
    <tr>
        <td style="width: 55%; padding-{{ $isAr ? 'left' : 'right' }}: 25px;">
            @if($salesInvoice->notes)
                ...
                <div style="font-size: 10px; color: #475569; line-height: 1.6;">{!! nl2br(e($salesInvoice->notes)) !!}</div>
            @endif
        </td>
            ...
            <div class="totals-box-wrapper">
                <table class="totals-summary-table">
                    <tr>
                        ...
                        <td class="summary-value">{{ number_format($salesInvoice->subtotal, 2) }} <span style="color: #94a3b8; font-size: 8px;">{{ $salesInvoice->currency }}</span></td>
                    </tr>
                    @if($salesInvoice->total_discount > 0)
                    <tr>
                        <td class="summary-label">{{ $isAr ? 'الخصم' : 'Discount' }}</td>
                        <td class="summary-value" style="color: #dc2626;">- {{ number_format($salesInvoice->total_discount, 2) }} <span style="color: #fca5a5; font-size: 8px;">{{ $salesInvoice->currency }}</span></td>
                    </tr>
                    @endif
                    @if($salesInvoice->tax_amount > 0)
                    <tr>
                        <td class="summary-label">{{ $isAr ? 'الضريبة' : 'Tax' }}</td>
                        <td class="summary-value">+ {{ number_format($salesInvoice->tax_amount, 2) }} <span style="color: #94a3b8; font-size: 8px;">{{ $salesInvoice->currency }}</span></td>
                    </tr>
                    @endif
                    <tr class="grand-total-row">
                        ...
                        <td class="summary-value">
                            ...
                            <span style="font-size: 9px; font-weight: normal; opacity: 0.85;">{{ $salesInvoice->currency }}</span>
                        </td>
                    </tr>
                </table>
            </div>
...
<table class="signature-layout">
    <tr>
        <td>
            ...
            <div class="signature-title-label">{{ $isAr ? 'توقيع المستلم' : 'Receiver Signature' }}</div>
        </td>
        <td>
            <div class="signature-dash-line"></div>
            <div class="signature-title-label">{{ $isAr ? 'توقيع المحاسب' : 'Accountant Signature' }}</div>
        </td>
    </tr>
</table>
...
<table class="footer-layout">
    <tr>
        <td style="text-align: {{ $isAr ? 'right' : 'left' }}; font-family: monospace;">
            {{ $salesInvoice->invoice_number }} | {{ optional($salesInvoice->invoice_date)->format('Y-m-d') }}
        </td>
...
<div style="text-align: center; margin-top: 18px;" dir="ltr">
    <barcode code="{{ $salesInvoice->invoice_number }}" type="C128B" size="0.9" height="0.7" />
