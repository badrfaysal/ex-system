...
<div class="max-w-3xl mx-auto animate-fade-in">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="divide-y divide-gray-100">
            @forelse($salesOrders as $so)
                <a href="{{ route('purchase-invoices.create', ['sales_order_id' => $so->id]) }}"
                   class="flex items-center justify-between px-6 py-4 hover:bg-blue-50/50 transition-colors">
                    <div>
                    ...
                    <div class="text-{{ $isAr ? 'left' : 'right' }}">
                        <p class="font-bold text-[#005B9F]" dir="ltr">{{ number_format($so->grand_total, 2) }} {{ $so->currency }}</p>
    ...
    <div class="mt-4">
        <a href="{{ route('purchase-invoices.index') }}" class="text-sm text-gray-500 hover:text-[#005B9F]">
            <i class="fas fa-arrow-{{ $isAr ? 'right' : 'left' }}"></i> {{ $isAr ? 'رجوع لفواتير الشراء' : 'Back to purchase invoices' }}
        </a>
    </div>
</div>
@endsection
