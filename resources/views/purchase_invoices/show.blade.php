    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
        ...
        <div class="px-8 py-4 flex justify-end">
            <div class="w-72 rounded-xl border border-gray-200 overflow-hidden text-sm">
                ...
                @php
                    $itemDiscounts = $purchaseInvoice->total_discount - $purchaseInvoice->extra_discount;
                @endphp
                @if($itemDiscounts > 0)
                ...
                @endif
                @if($purchaseInvoice->extra_discount > 0)
                <div class="flex justify-between items-center px-4 py-2.5 border-b border-gray-100 bg-red-50/20">
                    ...
                    <span class="font-bold text-red-600" dir="ltr">- {{ number_format($purchaseInvoice->extra_discount, 2) }} {{ $cur }}</span>
                </div>
                @endif
                @if($itemDiscounts > 0 && $purchaseInvoice->extra_discount > 0)
                <div class="flex justify-between items-center px-4 py-2.5 border-b border-gray-100 bg-red-50/30">
                    ...
                    <span class="font-bold text-red-600" dir="ltr">- {{ number_format($purchaseInvoice->total_discount, 2) }} {{ $cur }}</span>
                </div>
                @endif
                @if($purchaseInvoice->tax_amount > 0)
                <div class="flex justify-between items-center px-4 py-2.5 border-b border-gray-100">
                    <span class="text-gray-500">{{ $isAr ? 'إجمالي الضريبة' : 'Total Tax' }}</span>
                    <span class="font-bold text-gray-800" dir="ltr">+ {{ number_format($purchaseInvoice->tax_amount, 2) }} {{ $cur }}</span>
                </div>
                @endif
