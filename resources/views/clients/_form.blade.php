...
<div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
    ...
    <div>
        ...
        <select id="country" name="country" required
            class="w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:border-[#008A3B] focus:ring-1 focus:ring-[#008A3B] transition-colors bg-gray-50 focus:bg-white font-medium">
            <option value="" disabled {{ old('country', $entity?->country) ? '' : 'selected' }}>{{ __('messages.options.country.choose') ?? (app()->getLocale() === 'ar' ? 'اختر الدولة...' : 'Choose country...') }}</option>
            @php
                $countryList = isset($countries) && $countries->isNotEmpty()
                    ? $countries
                    : (\Illuminate\Support\Facades\Cache::get('system_settings')?->get('country') ?? \App\Models\Setting::where('category', 'country')->get());
            @endphp
            @foreach($countryList as $c)
                @php
                    $val = $c->key_value ?: $c->display_name;
                    $selected = old('country', $entity?->country) == $val || old('country', $entity?->country) == $c->display_name;
                @endphp
