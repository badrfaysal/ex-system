...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .title-main { font-size: 24px; font-weight: bold; color: #008A3B; margin: 0; line-height: 1.1; }
    ...
    .client-name { font-size: 15px; font-weight: bold; color: #0f172a; margin-bottom: 3px; }
    ...
    .type-order { color: #005B9F; font-weight: bold; }
    .type-receipt { color: #008A3B; font-weight: bold; }
    .amt-positive { color: #005B9F; font-weight: bold; }
    .amt-negative { color: #008A3B; font-weight: bold; }

    .final-box { background: #008A3B; color: #fff; border-radius: 6px; padding: 10px 16px; font-weight: bold; font-size: 13px; text-align: {{ $isAr ? 'left' : 'right' }}; margin-top: 10px; }
</style>
</head>
<body>
    <table class="top-global-bar"><tr><td style="background:#008A3B;"></td></tr></table>
    ...
    <table class="header-layout">
        <tr>
            ...
            <td style="width:40%; text-align:{{ $isAr ? 'left' : 'right' }};">
                <p class="title-main">{{ $isAr ? 'كشف حساب عميل' : 'Client Statement' }}</p>
    ...
    <table class="info-layout">
        <tr>
            <td style="width:55%;">
                <div class="block-label">{{ $isAr ? 'العميل' : 'CLIENT' }}</div>
                <div class="client-name">{{ $client->displayName($isAr ? 'ar' : 'en') }}</div>
                @if($client->phone || $client->email)
                <div style="font-size:10px;color:#64748b;">{{ $client->phone }} {{ $client->phone && $client->email ? '|' : '' }} {{ $client->email }}</div>
                @endif
            </td>
            <td style="width:45%;">
