...
<style>
  ...
  .top-bar { height: 5px; background: linear-gradient(90deg, #008A3B, #006e2e); }
  ...
  .highlight { background: #f0fdf4; border-{{ $isAr ? 'right' : 'left' }}: 4px solid #008A3B; border-radius: 4px; padding: 14px 18px; margin: 20px 0; }
...
<div class="card">
  ...
  <div class="body">
    @if($isAr)
      <p>السادة / {{ $client->displayName('ar') }}</p>
      <p>تحية طيبة وبعد،</p>
      <p>مرفق بهذا البريد كشف حساب مفصّل يوضح حركة حسابكم معنا.</p>
    @else
      <p>Dear {{ $client->displayName('en') }},</p>
      <p>Greetings,</p>
      <p>Please find attached your detailed account statement.</p>
    @endif

    <div class="highlight">
      <p>{{ $isAr ? 'الرصيد المستحق' : 'Balance Due' }}: <span class="grand">{{ number_format($balance, 2) }}</span></p>
    </div>
